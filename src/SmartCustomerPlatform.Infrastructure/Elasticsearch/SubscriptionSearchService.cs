        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (subscriptionId.HasValue)
        {
            filters.Add(new Query
            {
                Term = new TermQuery
                {
                    Field = Infer.Field<SubscriptionSearchResultDto>(x => x.Id),
        ...
        if (packageId.HasValue)
        {
            filters.Add(new Query
            {
                Term = new TermQuery
                {
                    Field = Infer.Field<SubscriptionSearchResultDto>(x => x.PackageId),
        ...
        if (!string.IsNullOrWhiteSpace(status))
        {
            filters.Add(new Query
            {
                Term = new TermQuery
                {
                    Field = Infer.Field<SubscriptionSearchResultDto>(x => x.Status),
                    Value = status
                }
            });
        }
        ...
        var request = new SearchRequest<SubscriptionSearchResultDto>(
            IndexName)
        {
            Size = 100,
            Query = query
